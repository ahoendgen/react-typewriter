...
class TypeWriter extends React.Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        const childrenAreStrings  = typeof children === 'string' &&
                                    typeof nextChildren === 'string';
        // TODO Implement childrenChanged for non-string children as well
        const childrenChanged     = childrenAreStrings && children !==
                                    nextChildren;
        const visibleCharsChanged = this.state.visibleChars !==
                                    nextState.visibleChars;
...
TypeWriter.propTypes = {
    ...
    typing(props, propName) {
        ...
        if (!(Number(prop) === prop && prop % 1 === 0) ||
            (prop < -1 || prop > 1)) {
            return new Error(
                'typing property must be an integer between 1 and -1');
        }
    },
